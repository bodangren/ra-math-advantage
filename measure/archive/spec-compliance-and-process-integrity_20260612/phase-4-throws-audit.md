...
> **Status:** Green implementation complete. Reviewed and corrected 2026-06-28.
>
...
| Metric | Count |
|--------|-------|
| Throw sites in scope (line-start `^\s*throw\b` + inline `if (...) throw`) | 83 |
| Throw sites in test files (`__tests__/`) — excluded | 2 |
| Throw sites processed by Green | 81 |
...
| `@throws` added by review-A correction (inline throws in `student.ts`, `rateLimits.ts`) | 9 |
| Throw sites without `@throws` after review-A correction | 0 |

> **Review-A correction (2026-06-28):** the original audit's throw-site detection
> regex `^\s*throw\b` only matched throw statements at the start of a line (with
> optional leading whitespace), missing 9 inline throw sites (`if (!x) throw ...`
> pattern) in 3 files: `student.ts` (6 inline throws in `completePhase` and
> `skipPhase`), `rateLimits.ts` (2 inline throws in `checkAndIncrementRateLimit`),
> and `dev.ts` (1 inline throw in `updateComponentStatus` — already covered by
> existing `@throws`). The 3 exported wrappers were missing `@throws`; review-A
> added them.
...
| apps/integrated-math-3/convex/dev.ts | 3 | 0 | 3 |
...
| apps/integrated-math-3/convex/rateLimits.ts | 3 | 3 | 0 |
...
| apps/integrated-math-3/convex/student.ts | 6 | 6 | 0 |
