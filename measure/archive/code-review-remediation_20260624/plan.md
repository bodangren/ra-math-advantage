...
- [x] Task: Add the JSDoc balanced-brace guard (FR-3) [evidence: e195fded]
    - [x] Red: add a test/lint fixture with a malformed `@returns {x {} …` and an unbalanced `@param {…}`; assert the guard fails on it [evidence: e195fded]
    ...
    - [x] Green: implement the guard (lint rule or test) scanning changed `*.ts`/`*.tsx` for unbalanced `{`/`}` and stray ` {} ` in `@param`/`@returns` [evidence: e195fded]
...
- [x] Task: Fix the 4 committed malformed `@returns` at HEAD (FR-1) [evidence: d26ecd52]
    ...
    - [x] `apps/integrated-math-3/app/api/dev/review-queue/route.ts:141` [evidence: e195fded]
    ...
    - [x] `apps/integrated-math-3/app/api/student/lesson-chatbot/route.ts:24` [evidence: e195fded]
        - [STRATEGY] Target: `@returns {string} The sanitized input safe for inclusion in an AI prompt.`
        - [GREEN EVIDENCE 2026-06-24, commit d26ecd52] Rewrote to canonical form. Per-file guard: violations=0, exit 0.
    - [x] `apps/integrated-math-3/components/teacher/gradebook/CourseOverviewGrid.tsx:16` [evidence: e195fded]
        - [STRATEGY] Target: `@returns {CourseOverviewRow[]} Sorted rows array.`
        - [GREEN EVIDENCE 2026-06-24, commit d26ecd52] Rewrote to canonical form. Per-file guard: violations=0, exit 0.
    - [x] `packages/knowledge-space-practice/src/projections/activity-map.ts:65` [evidence: e195fded]
        - [STRATEGY] Target: `@returns {ProjectedActivity[]} Sorted array of projected activities.`
        - [GREEN EVIDENCE 2026-06-24, commit d26ecd52] Rewrote to canonical form. Per-file guard: violations=0, exit 0.
- [x] Task: Discard & cleanly regenerate the uncommitted JSDoc batch (FR-2) [evidence: 0006074f]
    - [x] `git restore` the 144 working-tree files to HEAD (capture the list first for the record) [evidence: e195fded]
    ...
    - [x] Fix the JSDoc generator's `@returns` template (`{<type>}`, no trailing ` {}`) and `@param` inline object/function-type handling (fully balanced or prose-only) [evidence: e195fded]
    ...
    - [x] Re-run the generator; verify zero unbalanced-brace annotations via the FR-3 guard before committing [evidence: e195fded]
...
- [x] Task: Measure - User Manual Verification 'Phase 1: Malformed JSDoc remediation' (Protocol in workflow.md) [evidence: e195fded]
...
- [x] Task: Generalize student & parent projections to all modules (FR-4) [evidence: a826583c]
    - [x] Red: test `getStudentVisualizationHandler` + `projectParentVisualizationHandler` with multi-module placement rows; assert nodes from >1 module appear. The oracle must NOT be a module-1-only `projectStudentVisualization(module1Nodes, …)` re-implementation (that parity test cannot catch the scope bug — see FR-17) [evidence: a826583c]
    ...
    - [x] Green: add a shared full-curriculum graph-loading helper; replace the hardcoded `module-1/*.json` imports in `convex/student.ts` and `convex/parent/visualization.ts` [evidence: a826583c]
...
- [x] Task: Remove the throwaway `student_competency` read + replace its certifying test (FR-5, FR-16) [evidence: d8765ad1]
    - [x] Red+Green (atomic): DELETE the spy test + dead read + broken JSDoc in one commit. The spy test (`ctx.queryCalls.toContain('student_competency')`) was an anti-pattern per FR-16/FR-20. No replacement test needed — the dead read contributed nothing to observable output. [evidence: a826583c]
...
- [x] Task: Resolve the never-produced `review_due` state (FR-6) [evidence: d8765ad1]
    - [x] Red: test the intended `review_due` derivation, or assert the narrowed union [evidence: a826583c]
    ...
    - [x] Green: derive `review_due` from review/SRS data, or narrow the union type [evidence: a826583c]
...
- [x] Task: Measure - User Manual Verification 'Phase 2: Production-wiring scope & dead work' (Protocol in workflow.md) [evidence: 6c60fc92]
...
- [x] Task: De-duplicate generator utilities (FR-9) [evidence: 6c60fc92]
    - [x] Green: extract `seededRandom`, `generateCoefficients`, `formatPolynomial` into `packages/math-content/src/utils/`; update all 6/2/2 call sites; tests unchanged (determinism preserved) [evidence: 6c60fc92]
...
- [x] Task: Fix rational-analyzer HA triviality & grading + replace certifying test (FR-7, FR-16) [evidence: 4cbf9236]
    - [x] Red+Green (atomic): REPLACE the `rational-analyzer.test.ts` HA block + introduce non-monic variation in `rational-analyzer.ts` + change `advanced-math-adapters.ts` HA grading to scalar [evidence: 6c60fc92]
...
- [x] Task: Remove the exp-log dead domain re-roll + replace certifying tests (FR-8, FR-16) [evidence: 1ff4f012]
    - [x] Red+Green (atomic): REPLACE the `exp-log-solver.test.ts` domain blocks + delete `while(true)` loop + single-pass test [evidence: 6c60fc92]
...
- [x] Task: Consolidate registries & wire adapter nodeIds (FR-10, FR-18) [evidence: cb6e7f8c]
    - [x] Red+Green (atomic): add nodeIds assertion test + populate adapter nodeIds + remove flat registry [evidence: 6c60fc92]
...
- [x] Task: Correct PRNG labelling (FR-11) [evidence: 6c60fc92]
    - [x] Green: fix the docstring (subsumed by FR-9 shared util) [evidence: 6c60fc92]
...
- [x] Task: Measure - User Manual Verification 'Phase 3: advanced-math-generators correctness & quality' (Protocol in workflow.md) [evidence: cb6e7f8c]
...
- [x] Task: Fix the remediation script scan path & re-run (FR-12, FR-18) [evidence: 56183bd1]
    - [x] Red: add a test that the scanner finds concept blueprints in the **real** artifact (`curriculum/implementation/practice-v1/activity-map.json`) — there is currently no test for the script, so its "0 files" false-clean was invisible. Test must fail against the wrong-path version [evidence: 56183bd1]
    ...
    - [x] Green: point `scripts/remediate-concept-blueprints.ts` at the real blueprint artifact(s); re-run; record the true count [evidence: 56183bd1]
...
- [x] Task: Address concept resolution dropping sibling skills + replace certifying test (FR-13, FR-18) [evidence: 56183bd1]
    ...
    - [x] Green: emit rows for all child skills, or document single-skill resolution as intentional with rationale (code + comment accordingly) [evidence: 56183bd1]
...
- [x] Task: Measure - User Manual Verification 'Phase 4: precalc concept taxonomy' (Protocol in workflow.md) [evidence: 56183bd1]
...
- [x] Task: Finish IM3 auth-wrapper unification (FR-14) [evidence: b3128ce1]
    - [x] Red: update the IM3 auth test harness to stub the new `@math-platform/core-auth` request-guard exports; keep all IM3 auth tests green [evidence: b3128ce1]
    - [x] Green: replace inline `getCookieValueFromHeader` + response builders + guards in `apps/integrated-math-3/lib/auth/server.ts` with core-auth composition (mirror BM2) [evidence: b3128ce1]
    - [x] Resolve the matching tech-debt entry when complete [evidence: b3128ce1]
...
- [x] Task: Replace the planner grep-contract with a behavioral path test (FR-17) [evidence: a826583c, 503f9b4c]
    - [x] Red: add a behavioral test rendering the student dashboard from multi-module placement fixtures, asserting recommendations from >1 module surface (pairs with FR-4). Must fail against the module-1-only code [evidence: 503f9b4c]
    - [x] Green: keep `planner-prod-wiring.test.ts`'s source-scan only as a *complementary* architecture lint — it must no longer be the sole evidence for FR-5's "production consumer exists" [evidence: 503f9b4c]
- [x] Task: Strengthen generator-output & export tests (FR-19) [evidence: 503f9b4c]
    - [x] Add ≥1 test per generator validating `expectedAnswer`/`gradingMetadata` against the actual math (e.g. polynomial-operations result equals the computed operation; exp answer solves the equation) [evidence: 503f9b4c]
    - [x] Fix the seed-523 golden test's self-contradicting comment; derive its expected value from the operation, not from observed output [evidence: 503f9b4c]
    - [x] Replace the vacuous `games-exports.test.ts` type assertion (`{} as MatchingGameProps`) with a compile-time type check (`expectTypeOf`/`satisfies`) or remove it; keep the function-existence checks [evidence: 503f9b4c]
- [x] Task: Document the test-integrity standard (FR-20) [evidence: 503f9b4c]
    - [x] Add a `lessons-learned.md` entry: spy/grep/parity-oracle assertions standing in for behavioral coverage, and encoding a known limitation as the expected requirement, do not satisfy a behavioral FR [evidence: 503f9b4c]
- [x] Task: Measure - User Manual Verification 'Phase 7: Test integrity' (Protocol in workflow.md) [acceptance: 979dc945]

**Phase 7 acceptance evidence (2026-06-28, commit `979dc945`):** phase-acceptance verified FR-17/FR-19/FR-20 against implementation and live commands. `studentVisualizationMultiModule` passes 6/6; dashboard planner render tests pass 14/14; `planner-prod-wiring` remains only complementary architecture lint and passes 3/3; `generator-registry` passes 18/18; adapter `nodeIds` regression tests pass 47/47; `games-exports` compile-time type export tests pass 4/4; FR-20 lessons-learned entry present. Supervisor integrity checks: task regexes use `[~xb]`, `is_task_structurally_blocked` recognizes `[b]` and `deferred:<owner>`, and phase gate logic reports INCOMPLETE for zero-task phases. Root closeout gate blockers reported during Phase 7 were remediated in `0b2e5e18`; track-level final acceptance rerun revalidated `npm run lint`, `npx tsc --noEmit`, `npm run test`, and `bash measure/doctor.sh` as green.
...
- [x] Task: Remove stray junk files (FR-15) [green: ee51d9c9]
    - [x] Delete `--db` and `--symbol`; add `.gitignore` guard if the pattern can recur [evidence: ee51d9c9]
- [x] Task: Update tech-debt registry [green: dcb35dbb]
    - [x] Mark FR-14 auth item Resolved; add documented entries for any FR converted to deferred debt (per Acceptance Criterion 13) [evidence: ee51d9c9]
- [x] Task: Generate Docs & Doctor [acceptance: 19a8a24a]
    - [x] Run `scripts/generate-measure-docs.ts`; refresh `graph.db` if signatures/exports changed [evidence: ee51d9c9]
