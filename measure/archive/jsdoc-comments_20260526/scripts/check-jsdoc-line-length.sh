#!/usr/bin/env bash
# Phase 1 JSDoc Line-Length Guard — Red baseline supplement for jsdoc-comments_20260526.
#
...
#
# Sibling to check-jsdoc-coverage.sh:
#   * check-jsdoc-coverage.sh : structural — "every function has a summary in graph.db"
#   * check-jsdoc-line-length.sh (this file) : prose-shape — "no JSDoc continuation line > 120 chars"
#
...
#   SCOPE_DIR  : root directory to scan (default apps/bus-math-v2/lib)

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)"
MAX_CHARS="${MAX_CHARS:-120}"
SCOPE_DIR="${SCOPE_DIR:-${REPO_ROOT}/apps/bus-math-v2/lib}"
