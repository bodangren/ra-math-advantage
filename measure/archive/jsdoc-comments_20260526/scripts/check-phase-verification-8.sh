#!/usr/bin/env bash
# Phase 8 Manual Verification Guard — Red baseline for the
# `Measure - User Manual Verification 'Phase 8: Packages src/'` task in plan.md.
#
...
# existing Phase 8 guards:
#
#   * check-jsdoc-coverage-packages-src.sh    : structural  — every function has a summary in graph.db
#   * check-jsdoc-line-length-packages-src.sh : prose-shape — no JSDoc continuation line > 120 chars
#   * check-phase-verification-8.sh (this)    : process     — user manual verification has been recorded
#
...
# (Phase 4), check-phase-verification-5.sh (Phase 5), check-phase-verification-6.sh
# (Phase 6), and check-phase-verification-7.sh (Phase 7). Same shape, different report
...
#
# Asserts: phase-8-verification-report.md exists, reports `VERIFICATION_RESULT: approved`,
...
#
# Usage:
#   bash measure/tracks/jsdoc-comments_20260526/scripts/check-phase-verification-8.sh
#   bash measure/tracks/jsdoc-comments_20260526/scripts/check-phase-verification-8.sh --json

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)"
REPORT="${REPO_ROOT}/measure/tracks/jsdoc-comments_20260526/phase-8-verification-report.md"
...
if [ ! -f "${REPORT}" ]; then
    if [ "${JSON_MODE}" = "1" ]; then
        ...
        echo "ERROR: phase-8-verification-report.md not found at:" >&2
        echo "  ${REPORT}" >&2
        echo "" >&2
...
if [ "${JSON_MODE}" = "1" ]; then
    ...
    echo ""
    if [ "${PASS}" = "true" ]; then
        echo "PASS: Phase 8 manual verification has been recorded as approved."
    else
        echo "FAIL: Phase 8 manual verification has not been completed."
        echo ""
        echo "Open issues:"
        for r in "${REASONS[@]}"; do
            echo "  - ${r}"
        ...
        echo "of phase-8-verification-report.md with the approved result + verifier + timestamp."
    fi
fi
