#!/usr/bin/env bash
# Phase 5 Manual Verification Guard — Red baseline for the
# `Measure - User Manual Verification 'Phase 5: IM3 components/'` task in plan.md.
#
...
# existing Phase 5 guards:
#
#   * check-jsdoc-coverage-components-im3.sh      : structural  — every function has a summary in graph.db
#   * check-jsdoc-line-length-components-im3.sh   : prose-shape — no JSDoc continuation line > 120 chars
#   * check-phase-verification-5.sh (this file)   : process     — user manual verification has been recorded
#
# Sibling of check-phase-verification.sh (Phase 1), check-phase-verification-2.sh
# (Phase 2), check-phase-verification-3.sh (Phase 3), and check-phase-verification-4.sh
# (Phase 4). Same shape, different report path — kept as a separate file so the
# per-phase acceptance gate stays a single command and so the JSON output reports the
# right phase label.
#
# Asserts: phase-5-verification-report.md exists, reports `VERIFICATION_RESULT: approved`,
...
REPORT="${REPO_ROOT}/measure/tracks/jsdoc-comments_20260526/phase-5-verification-report.md"
...
if [ ! -f "${REPORT}" ]; then
    if [ "${JSON_MODE}" = "1" ]; then
        ...
        echo "ERROR: phase-5-verification-report.md not found at:" >&2
        echo "  ${REPORT}" >&2
        echo "" >&2
...
if [ "${JSON_MODE}" = "1" ]; then
    ...
    echo ""
    if [ "${PASS}" = "true" ]; then
        echo "PASS: Phase 5 manual verification has been recorded as approved."
    else
        echo "FAIL: Phase 5 manual verification has not been completed."
        echo ""
        echo "Open issues:"
        for r in "${REASONS[@]}"; do
            echo "  - ${r}"
        ...
        echo "of phase-5-verification-report.md with the approved result + verifier + timestamp."
    fi
fi
