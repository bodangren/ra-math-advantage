#!/usr/bin/env bash
# Phase 2 JSDoc Coverage Guard — Red baseline test for jsdoc-comments_20260526.
#
...
#
# Asserts: zero functions in apps/bus-math-v2/components/** have NULL summaries in graph.db.
# Exit 0 = pass (Phase 2 acceptance met). Non-zero = fail (functions still undocumented).
#
# This is the Phase 2 sibling of check-jsdoc-coverage.sh (which targets Phase 1's BM2 lib/).
# Same SQL shape, different scope — kept as a separate file so the per-phase acceptance
# gate stays a single command and so the JSON output reports the right phase label.
#
# Usage:
...
#
# Requires: build-graph on PATH, ./graph.db at repo root, BM2 components/ files scanned.
...
SCOPE_PATTERN="%/apps/bus-math-v2/components/%"
SCOPE_LABEL="apps/bus-math-v2/components/"
...
EOF
    ...
    echo "  - exported:             ${NULL_EXPORTED}  (Task 2.1 target)"
    echo "  - internal:             ${NULL_INTERNAL}  (Task 2.2 target)"
    echo ""
fi
...
if [ "${JSON_MODE}" = "0" ]; then
    ...
    echo ""
    echo "Per-function list (top 25 NULL files):"
    build-graph query "${GRAPH_DB}" \
        "SELECT file_path, COUNT(*) AS n FROM nodes WHERE type='function' AND file_path LIKE '${SCOPE_PATTERN}' AND summary IS NULL GROUP BY file_path ORDER BY n DESC LIMIT 25" 2>/dev/null || true
    echo ""
