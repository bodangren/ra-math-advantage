...
- [x] **Track: Spec Compliance and Process Integrity Remediation** — **COMPLETED / ARCHIVED** (2026-06-30)
   *Adversarial review of the past 24 hours found systematic spec violations, automation-self-approved "manual" verification, FR-6 source mutations, missing FR-5 type annotations, and a hazardous repository state (detached HEAD, 28 stashes, dirty graph.db). This track fixes every identified issue before any dependent track proceeds. All 7 phases complete: repository state triage, FR-6 reverts, FR-5 typed annotations across IM3 and packages, exported-surface JSDoc coverage, verification process integrity, kst-lesser-holes Phase 1 quality gates, and final checkpoint. Final Green commit: 3d4243e8.*
   *Link: [./archive/spec-compliance-and-process-integrity_20260612/](./archive/spec-compliance-and-process-integrity_20260612/)*
...
- [x] **Track: Dependency Security & Package Upgrades** — **COMPLETED** *(Archived 2026-06-13)*
...
- [x] **Track: Teacher Content Authoring** — **COMPLETED (acceptance passed; closeout pending)**
   *Tools for teachers to create/edit lesson content and activities, not just assign existing ones, gated by the component-approval workflow and activity schemas. All 4 phases complete with commit-SHA evidence: Phase 1 authoring model + schema validation + sanitizer (7516f07c; Review B sanitizer hardening e51561e4); Phase 2 Convex draft lifecycle + approval/hash integration + teacher-scoped authz (01752a0c); Phase 3 composer UI + preview + status surfacing (4e43965b; UX Preview-button fix 11745f94); Phase 4 E2E lifecycle adapter + final verification (e347bf98; Review B deep-clone 15e5154a; Review C displayName fallback 705e0cc3; Phase 4 acceptance verdict 4b67ee73; Phase 4 adversarial eeab6a41 — 13 new tests). FR1–FR6 met via the passing E2E lifecycle (content-authoring-e2e.test.tsx). Final-acceptance verdict: PASS (final-acceptance-result.json). The 4 per-phase UMV tasks remain structurally `[b] deferred:user` (human-gated; do not block automated closeout). Pre-existing Phase 3 jsdom timeout flakes (LessonComposer-adversarial, LessonComposer-preview-button) are documented in plan.md and exit-code-truthful; 0 Phase 4 regressions. Track remains in `measure/tracks/` pending the `measure-closeout` archive move.*
   *Link: [./tracks/teacher-content-authoring_20260605/](./tracks/teacher-content-authoring_20260605/)*

## Archived Tracks

- [x] **[HIGH PRIORITY] Track: Code Review Remediation (2026-06-24)** — **COMPLETED / ARCHIVED** (2026-06-28)
   *Remediates every defect found in the line-by-line review of the last 72h of commits: 4 committed + ~345 uncommitted malformed JSDoc annotations (discard & cleanly regenerate + add a balanced-brace guard), module-1-only student/parent projections (generalize to all modules), the planner's throwaway `student_competency` read, rational-analyzer HA triviality + object grading, exp-log dead re-roll, 5× duplicated generator PRNG, two competing `GENERATOR_REGISTRY` + empty adapter `nodeIds`, the precalc remediation script scanning the wrong filename (false "clean"), the deferred IM3 auth-wrapper unification, and stray `--db`/`--symbol` junk files. Includes a **test-integrity** cluster: replacing the spy/grep/parity-oracle tests that certified the defects with behavioral tests. Phases 1–8 substantive work and acceptance complete (commits e195fded..19a8a24a); repo-level closeout gate blockers remediated in 0b2e5e18; final acceptance passed and track archived.*
   *Link: [./measure/archive/code-review-remediation_20260624/](./measure/archive/code-review-remediation_20260624/)*
...
- [x] **Track: Parent Portal Production Wiring Remediation** — **COMPLETED / ARCHIVED** (2026-06-23)
   *Remediation of the 2026-06-21 completion-audit finding. Replaced the static `/parent` stub with a live async server component calling `requireParentServerSessionClaims`, added `convex/parent/visualization.ts` (parent-safe projection query), wired `ParentDashboard`/`StudentSwitcher`/`ParentEmptyStates` from live route data, and added route-rendering tests proving non-test callers exist. 7/7 track tests + 44/44 parent component tests + 25/25 parent convex tests + 45/45 parent auth tests passing. All 6 FRs satisfied. Re-mediates parent-portal_20260605.*
   *Link: [./measure/archive/parent_portal_prod_wiring_remediation_20260621/](./measure/archive/parent_portal_prod_wiring_remediation_20260621/)*
