...
| Cross-vocabulary validation gap | Medium | Open | `validateKnowledgeSpace` errors on standard/renderer/generator node references because those registries are not merged with course draft-nodes during validation. Need a merged-registry validation mode or a shared vocabulary node file. IM1: 941, IM2: 938, IM3: 903 dangling edges from this gap. |
| IM3 M1 pilot duplicate edges (14) | Low | Open | `module-1/edges.json` contains 14 duplicate (sourceId,targetId,type) triples for rendered_by edges. Pre-existing; harmless for draft use but will fail strict duplicate checks. Fix before promoting edges to reviewed status. |
