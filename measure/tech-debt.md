...
| Track 5 P2: `buildChain3()`/`buildBranch()` bare node IDs fail `CORE_ID_PATTERN` | High | Resolved | Helpers changed to dot-separated IDs (`math.test.a` etc.); adapter presets and assertions updated. 2 tests fixed. Source: Phase 2 green (81307209). |
| Track 5 P2: "fail-all walks back" adapter preset mismatch | High | Resolved | Adapter preset corrected to `{a:'fail', b:'fail', c:'fail'}`. 1 test fixed. Source: Phase 2 green (81307209). |
...
| Track 5 P2: `runPlacementTraversal` does not await async `ProbeAdapter.probe` | High | Resolved | Engine made `async`; `adapter.probe(nodeId)` now `await`ed. All call sites updated with `await`. 2 async probe tests pass. Source: Phase 2 green (81307209). |
| Track 5 P3: `__tests__/lib/placement/im3-probe-adapter.test.ts` Task 3.1.e end-to-end tests do not `await` `runPlacementTraversal` | Medium | Open | Pre-existing tests on lines ~303–378 (Task 3.1.e) treat `runPlacementTraversal` as sync, contradicting the engine's own test pattern (`packages/knowledge-space-core/src/__tests__/placement-engine.test.ts`) which uses `async` + `await`. New Red-phase tests in Task 3.1.g/3.1.h/3.1.i added in 2026-06-05 follow the correct async/await pattern. Green-phase implementer must fix the Task 3.1.e tests by adding `async` to `it()` callbacks and `await` to `runPlacementTraversal(...)` calls before Task 3.1 can be marked complete. Source: review of 2026-06-05. |
