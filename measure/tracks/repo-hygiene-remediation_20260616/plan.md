...
- [~] Task 3.1b: Implement role-aware Dashboard link in `<UserMenu>` (deferred from Task 3.1) — Red phase owned by this attempt
  ...
  - [ ] Verify: 9/9 tests in `user-menu.test.tsx` pass, plus 3 new Red cases in `user-menu-dashboard-link.test.tsx` turn green.
...
- [~] Task 3.2: Fix GradebookDrillDown flaky timeout — Red phase owned by this attempt
  ...
  - [ ] Green fix options (deferred): (a) make the modal loader carry `role="dialog"` so the dialog is present from the first render; (b) bump `waitFor` timeout in the integration test; (c) mock `fetchInternalQuery` so the dynamic-import resolution becomes synchronous. Test-strategy.md §3.2 prefers (a) or (c) over (b).
  - [ ] Verify test passes consistently under the default 5 s vitest timeout.
