...
- [~] Task: Implement deterministic generators for the prioritized IM1 skills, reusing T17–T19 mechanisms (TDD)
  - Red tests landed (one file per Module-1 skill, the vertical-slice module locked in `metadata.json:verticalSliceModule = "1"`):
    - `__tests__/_pending/1-1-verbal-to-numerical.pending.test.ts` — `math.im1.skill.1.1.translate-verbal-descriptions-into-correct-numerical-express`
    - `__tests__/_pending/1-2-algebraic-verbal-translation.pending.test.ts` — `math.im1.skill.1.2.translate-between-algebraic-expressions-and-verbal-expressio`
    - `__tests__/_pending/1-3-properties-of-equality.pending.test.ts` — `math.im1.skill.1.3.identify-and-apply-the-reflexive-symmetric-and-transitive-pr`
    - `__tests__/_pending/1-4-distributive-property.pending.test.ts` — `math.im1.skill.1.4.apply-the-distributive-property-to-rewrite-and-evaluate-nume`
    - `__tests__/_pending/1-5-absolute-value-distance.pending.test.ts` — `math.im1.skill.1.5.write-absolute-value-expressions-that-model-real-world-dista`
    - `__tests__/_pending/1-6-ratios-and-percentages.pending.test.ts` — `math.im1.skill.1.6.use-ratios-and-percentages-as-metrics-to-model-real-world-si`
  ...
  - Implementer (Green) handoff for this task: implement each per-skill generator (deterministic, seed-driven, real math — NOT `createStubGenerator`), register in `IM1_GENERATORS`, and **move the corresponding `.pending.test.ts` out of `_pending/` in the same commit** (test-strategy §8 "one task ↔ one file"). The vitest `exclude` from Task 1 ensures `_pending/**` is invisible to the aggregate suite once active.
- [~] Task: All new generators pass the Generated-Math Correctness QA harness (golden-answer + properties)
  ...
  - Implementer (Green) handoff for this task: this gate is the closeout for Phase 2 — it cannot pass until Task 1 (registry wiring) and every Task 2 sub-task (real generator per Module-1 skill) are landed. Wire the IM1 ci-gate into root CI alongside `npm run -w packages/practice-core test:generators` so Phase 5 audit-refresh can rely on a single green gate per course.
- Phase 2 Red-phase aggregate: 1 commit; targeted command `npm run -w packages/math-content test -- problem-families/im1` reports **Test Files 8 failed | 1 passed (9); Tests 18 passed (18)** at HEAD. The 18 passing tests are the existing Phase 1 coverage-matrix suite — confirms Red scoping did not regress earlier work. Exit code 1, suitable for CI.
