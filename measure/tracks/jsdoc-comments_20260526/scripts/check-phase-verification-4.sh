#!/usr/bin/env bash
# Phase 4 Manual Verification Guard — Red baseline for the
# `Measure - User Manual Verification 'Phase 4: IM3 convex/'` task in plan.md.
#
...
#
#   * check-jsdoc-coverage-convex-im3.sh    : structural  — every function has a summary in graph.db
#   * check-jsdoc-line-length-convex-im3.sh : prose-shape — no JSDoc continuation line > 120 chars
#   * check-phase-verification-4.sh (this)  : process     — user manual verification has been recorded
#
# Sibling of check-phase-verification.sh (Phase 1), check-phase-verification-2.sh
# (Phase 2), and check-phase-verification-3.sh (Phase 3). Same shape, different report
...
#
# Asserts: phase-4-verification-report.md exists, reports `VERIFICATION_RESULT: approved`,
...
REPORT="${REPO_ROOT}/measure/tracks/jsdoc-comments_20260526/phase-4-verification-report.md"
...
if [ ! -f "${REPORT}" ]; then
    if [ "${JSON_MODE}" = "1" ]; then
        ...
        echo "ERROR: phase-4-verification-report.md not found at:" >&2
        echo "  ${REPORT}" >&2
        echo "" >&2
...
if [ "${JSON_MODE}" = "1" ]; then
    ...
    echo ""
    if [ "${PASS}" = "true" ]; then
        echo "PASS: Phase 4 manual verification has been recorded as approved."
    else
        echo "FAIL: Phase 4 manual verification has not been completed."
        echo ""
        echo "Open issues:"
        for r in "${REASONS[@]}"; do
            echo "  - ${r}"
        ...
        echo "of phase-4-verification-report.md with the approved result + verifier + timestamp."
    fi
fi
