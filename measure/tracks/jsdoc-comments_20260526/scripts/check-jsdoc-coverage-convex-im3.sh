#!/usr/bin/env bash
# Phase 4 JSDoc Coverage Guard — Red baseline test for jsdoc-comments_20260526.
#
...
#
# Asserts: zero functions in apps/integrated-math-3/convex/** (excluding _generated/)
# have NULL summaries in graph.db.
# Exit 0 = pass (Phase 4 acceptance met). Non-zero = fail (functions still undocumented).
#
# This is the Phase 4 sibling of check-jsdoc-coverage-remaining.sh (Phase 3 BM2
# app/convex/scripts/other) and the per-app sibling of the IM3-targeted coverage
# guards in later phases. Same SQL shape, different scope — kept as a separate file
# so the per-phase acceptance gate stays a single command and so the JSON output
# reports the right phase label.
#
# Scope rationale: convex/_generated/ is generated Convex client code and is excluded
# from JSDoc scope per test-strategy.md §3 ("Convex generated code … must also be
# excluded from doc scope").
#
# Usage:
...
#
# Requires: build-graph on PATH, ./graph.db at repo root, IM3 convex/ files scanned.
...
SCOPE_PATTERN="%/apps/integrated-math-3/convex/%"
SCOPE_EXCLUDE_PATTERN="%/apps/integrated-math-3/convex/_generated/%"
SCOPE_LABEL="apps/integrated-math-3/convex/"
...
QUERY_TOTAL="SELECT COUNT(*) FROM nodes WHERE type='function' AND file_path LIKE '${SCOPE_PATTERN}' AND file_path NOT LIKE '${SCOPE_EXCLUDE_PATTERN}'"
QUERY_NULL="SELECT COUNT(*) FROM nodes WHERE type='function' AND file_path LIKE '${SCOPE_PATTERN}' AND file_path NOT LIKE '${SCOPE_EXCLUDE_PATTERN}' AND summary IS NULL"
QUERY_EXPORTED_NULL="SELECT COUNT(*) FROM nodes WHERE type='function' AND file_path LIKE '${SCOPE_PATTERN}' AND file_path NOT LIKE '${SCOPE_EXCLUDE_PATTERN}' AND tags LIKE '%exported%' AND summary IS NULL"
QUERY_INTERNAL_NULL="SELECT COUNT(*) FROM nodes WHERE type='function' AND file_path LIKE '${SCOPE_PATTERN}' AND file_path NOT LIKE '${SCOPE_EXCLUDE_PATTERN}' AND (tags NOT LIKE '%exported%' OR tags IS NULL) AND summary IS NULL"
QUERY_PER_FILE_NULL="SELECT file_path, COUNT(*) AS n FROM nodes WHERE type='function' AND file_path LIKE '${SCOPE_PATTERN}' AND file_path NOT LIKE '${SCOPE_EXCLUDE_PATTERN}' AND summary IS NULL GROUP BY file_path ORDER BY n DESC"
...
EOF
    ...
    echo "==========================================="
    echo "Scope:                    ${SCOPE_LABEL} (excluding _generated/)"
    echo "Total functions:          ${TOTAL}"
    echo "NULL summaries (total):   ${NULL_TOTAL}"
    echo "  - exported:             ${NULL_EXPORTED}  (Task 4.1 target)"
    echo "  - internal:             ${NULL_INTERNAL}  (Task 4.2 target)"
    echo ""
fi
