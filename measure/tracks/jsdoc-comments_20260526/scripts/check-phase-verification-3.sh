#!/usr/bin/env bash
# Phase 3 Manual Verification Guard — Red baseline for the
# `Measure - User Manual Verification 'Phase 3: BM2 remaining dirs'` task in plan.md.
#
...
# existing Phase 3 guards:
#
#   * check-jsdoc-coverage-remaining.sh      : structural  — every function has a summary in graph.db
#   * check-jsdoc-line-length-remaining.sh   : prose-shape — no JSDoc continuation line > 120 chars
#   * check-phase-verification-3.sh (this)  : process     — user manual verification has been recorded
#
# Sibling of check-phase-verification.sh (Phase 1) and check-phase-verification-2.sh
# (Phase 2). Same shape, different report path — kept as a separate file so the
# per-phase acceptance gate stays a single command and so the JSON output reports the
# right phase label.
#
# Asserts: phase-3-verification-report.md exists, reports `VERIFICATION_RESULT: approved`,
...
REPORT="${REPO_ROOT}/measure/tracks/jsdoc-comments_20260526/phase-3-verification-report.md"
...
if [ ! -f "${REPORT}" ]; then
    if [ "${JSON_MODE}" = "1" ]; then
        ...
        echo "ERROR: phase-3-verification-report.md not found at:" >&2
        echo "  ${REPORT}" >&2
        echo "" >&2
...
if [ "${JSON_MODE}" = "1" ]; then
    ...
    echo ""
    if [ "${PASS}" = "true" ]; then
        echo "PASS: Phase 3 manual verification has been recorded as approved."
    else
        echo "FAIL: Phase 3 manual verification has not been completed."
        echo ""
        echo "Open issues:"
        for r in "${REASONS[@]}"; do
            echo "  - ${r}"
        ...
        echo "of phase-3-verification-report.md with the approved result + verifier + timestamp."
    fi
fi
