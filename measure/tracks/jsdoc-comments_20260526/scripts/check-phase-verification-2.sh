#!/usr/bin/env bash
# Phase 2 Manual Verification Guard — Red baseline for the
# `Measure - User Manual Verification 'Phase 2: BM2 components/'` task in plan.md.
#
...
# existing Phase 2 guards:
#
#   * check-jsdoc-coverage-components.sh : structural  — every function has a summary in graph.db
#   * check-jsdoc-line-length-components.sh : prose-shape — no JSDoc continuation line > 120 chars
#   * check-phase-verification-2.sh (this file) : process — user manual verification has been recorded
#
# Sibling of check-phase-verification.sh (Phase 1). Same shape, different report path —
...
#
# Asserts: phase-2-verification-report.md exists, reports `VERIFICATION_RESULT: approved`,
...
REPORT="${REPO_ROOT}/measure/tracks/jsdoc-comments_20260526/phase-2-verification-report.md"
...
if [ ! -f "${REPORT}" ]; then
    if [ "${JSON_MODE}" = "1" ]; then
        ...
        echo "ERROR: phase-2-verification-report.md not found at:" >&2
        echo "  ${REPORT}" >&2
        echo "" >&2
...
if [ "${JSON_MODE}" = "1" ]; then
    ...
    echo ""
    if [ "${PASS}" = "true" ]; then
        echo "PASS: Phase 2 manual verification has been recorded as approved."
    else
        echo "FAIL: Phase 2 manual verification has not been completed."
        echo ""
        echo "Open issues:"
        for r in "${REASONS[@]}"; do
            echo "  - ${r}"
        ...
        echo "of phase-2-verification-report.md with the approved result + verifier + timestamp."
    fi
fi
