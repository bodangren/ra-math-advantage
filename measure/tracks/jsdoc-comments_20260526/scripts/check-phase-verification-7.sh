#!/usr/bin/env bash
# Phase 7 Manual Verification Guard — Red baseline for the
# `Measure - User Manual Verification 'Phase 7: IM3 app/scripts/other'` task in plan.md.
#
...
# existing Phase 7 guards:
#
#   * check-jsdoc-coverage-im3-app.sh        : structural  — every function has a summary in graph.db
#   * check-jsdoc-line-length-im3-app.sh     : prose-shape — no JSDoc continuation line > 120 chars
#   * check-phase-verification-7.sh (this)   : process     — user manual verification has been recorded
#
...
# (Phase 4), check-phase-verification-5.sh (Phase 5), and check-phase-verification-6.sh
# (Phase 6). Same shape, different report path — kept as a separate file so the per-phase
# acceptance gate stays a single command and so the JSON output reports the right phase label.
#
# Asserts: phase-7-verification-report.md exists, reports `VERIFICATION_RESULT: approved`,
...
#
# Usage:
#   bash measure/tracks/jsdoc-comments_20260526/scripts/check-phase-verification-7.sh
#   bash measure/tracks/jsdoc-comments_20260526/scripts/check-phase-verification-7.sh --json

set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../../.." && pwd)"
REPORT="${REPO_ROOT}/measure/tracks/jsdoc-comments_20260526/phase-7-verification-report.md"
...
if [ ! -f "${REPORT}" ]; then
    if [ "${JSON_MODE}" = "1" ]; then
        ...
        echo "ERROR: phase-7-verification-report.md not found at:" >&2
        echo "  ${REPORT}" >&2
        echo "" >&2
...
if [ "${JSON_MODE}" = "1" ]; then
    ...
    echo ""
    if [ "${PASS}" = "true" ]; then
        echo "PASS: Phase 7 manual verification has been recorded as approved."
    else
        echo "FAIL: Phase 7 manual verification has not been completed."
        echo ""
        echo "Open issues:"
        for r in "${REASONS[@]}"; do
            echo "  - ${r}"
        ...
        echo "of phase-7-verification-report.md with the approved result + verifier + timestamp."
    fi
fi
