#!/usr/bin/env bash
# Phase 5 JSDoc Coverage Guard — Red baseline test for jsdoc-comments_20260526.
#
...
#
# Asserts: zero functions in apps/integrated-math-3/components/** have NULL summaries
# in graph.db.
# Exit 0 = pass (Phase 5 acceptance met). Non-zero = fail (functions still undocumented).
#
# This is the Phase 5 IM3 sibling of check-jsdoc-coverage-components.sh (Phase 2 BM2
# components/) and the per-app sibling of the IM3-targeted coverage guards in later
# phases. Same SQL shape, different scope — kept as a separate file so the per-phase
# acceptance gate stays a single command and so the JSON output reports the right
# phase label.
#
# Phase 5 scope is intentionally narrow: apps/integrated-math-3/components/**. It does
# NOT include the apps/integrated-math-3/lib/, apps/integrated-math-3/convex/, or
# apps/integrated-math-3/app/ directories — those are covered by Phases 6, 4, and 7
# respectively. Phases are workspace-scoped per test-strategy.md §3 ("Phase dependency:
# Phases are workspace-scoped; failure in one phase does not block another.").
#
# Usage:
...
#
# Requires: build-graph on PATH, ./graph.db at repo root, IM3 components/ files scanned.
...
SCOPE_PATTERN="%/apps/integrated-math-3/components/%"
SCOPE_LABEL="apps/integrated-math-3/components/"
...
EOF
    ...
    echo "  - exported:             ${NULL_EXPORTED}  (Task 5.1 target)"
    echo "  - internal:             ${NULL_INTERNAL}  (Task 5.2 target)"
    echo ""
fi
