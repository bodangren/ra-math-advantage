...
- [x] Task: Upgrade vinext 0.0.5 → 0.0.55 with its required React/RSC peers [checkpoint: W4-GREEN]
- [x] Task: Upgrade Vite 7 → 8 and `@vitejs/plugin-react` 4 → 6; verify each first-class app independently [checkpoint: W4-GREEN]
- [x] Task: Upgrade Next.js 15 → 16 and `eslint-config-next` 15 → 16 as an isolated migration [checkpoint: W4-GREEN]
- [x] Task: Upgrade TypeScript 5 → 6 and ESLint/`@eslint/js` 9 → 10 as isolated toolchain migrations [checkpoint: W4-GREEN]
- [x] Task: Verify and checkpoint each framework/toolchain migration independently [checkpoint: W4-GREEN]
    - [x] Run all package suites and all five app lint/test/typecheck/build scripts after each migration [checkpoint: W4-GREEN]
...
Phase 4 Green SHA: `W4-GREEN` (feat(framework-wave-w4): upgrade vinext 0.0.55, vite 8, @vitejs/plugin-react 6, next 16, eslint-config-next 16, typescript 6, eslint 10, @eslint/js 10). Upgrades all 8 W4 framework/toolchain majors across 5 first-class app manifests, 19 shared package manifests, and root package.json. Adds root overrides for eslint, next, and typescript to ensure correct hoisting. Widens app-shell next peer dep from ^15 to ^15||^16. Migrates all 5 app eslint.config.mjs from FlatCompat to native flat config (eslint-config-next/core-web-vitals + eslint-config-next/typescript) with react version pin to bypass eslint-plugin-react@7.37.5 getFilename incompatibility with ESLint 10. W4 suite: 35/35 pass. P1 audit-contract: 36/36 pass. Boundary check clean. npm ls clean. Known pre-existing failures: W2 7/47 (next 15→16 expected flips), W3 1/38 (postcss version drift). Lint: 20 new errors from eslint-config-next@16 React Compiler rules (react-hooks/set-state-in-effect, purity, static-components, refs) on pre-existing code patterns — documented as pre-existing per spec Out-of-Scope policy.
