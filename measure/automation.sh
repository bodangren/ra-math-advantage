...
REVIEW_SLEEP_TIME=1800
WORK_SESSIONS=2
SESSION_TIMEOUT=3600  # 60 minutes in seconds

WORK_PROMPT="/measure @AGENTS.md Use the measure skill to complete one entire phase of the current or next track. Always sync with remote and deal with conflicts intelligently. If workspace has some documentation updates or previous edits, simply commit those before starting work. Work autonomously without any guidance from the user until the phase is complete. Make sure to run tests, npm run build (fixing any blockers, whether your code or not), then commit with note and push."

REVIEW_PROMPT="/measure @AGENTS.md You are an expert code-review consultant brought in to audit the past 6 work phases. Always sync with remote and deal with conflicts intelligently. If workspace has some documentation updates or previous edits, simply commit those before starting work. Work autonomously without any guidance from the user until the code review is complete. Fix any serious issues you find. Record all issues in the appropriate measure/ docs.  Make sure to run tests, npm run build (fixing any blockers, whether your code or not), update measure/current_directive.md with next high-level priorities, check README.md for correctness, then commit with note and push."
