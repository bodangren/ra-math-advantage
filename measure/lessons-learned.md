...
- (2026-04-29, review-29) TypeScript `as` casts are compile-time only — wrapping them in try/catch creates dead code that appears to handle errors but never fires. For Convex ID validation, use `v.id()` validators at the arg level instead of runtime casts. When exporting handler functions for testing, use the pattern `export async function fooHandler(ctx, args) { ... }` + `export const foo = internalMutation({ handler: fooHandler })` — this avoids `as any` casts in test files.
- (2026-04-29, review-29) Convex `v.string()` args accepting endpoint names should use `v.union(v.literal(...))` — unknown strings silently bypass rate limiting when the config lookup returns undefined. Always deny-by-default for security-sensitive validators.
- (2026-04-29, prompt-guard) Regex patterns with optional trailing groups (`?`) cause false positives on single keywords — require both action and target keywords with `.+` between them. Always test prompt guards against legitimate student questions containing common words like "ignore", "forget", "show", "previous".
