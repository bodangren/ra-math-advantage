...
- (2026-06-06, edge-calibration-red; recurred 4+× through 2026-06-07 review) `graph.db` is a tracked binary; running `build-graph stats` / `search` / `inspect` mutates it (file size + node counts drift). Run these from a scratch path (e.g. `cp graph.db /tmp/agent-graph.db` and read that) or `git restore graph.db` before committing. Never leave graph.db as a working-tree modification — supervisors reject it as a non-test/non-Measure change. **Enforced (2026-06-11): `scripts/git-hooks/pre-commit` blocks staging a modified `graph.db` (activate per clone with `git config core.hooksPath scripts/git-hooks`; override with `ALLOW_GRAPH_DB=1`). Note `build-graph query` is read-only — only stats/search/inspect mutate the DB.**
