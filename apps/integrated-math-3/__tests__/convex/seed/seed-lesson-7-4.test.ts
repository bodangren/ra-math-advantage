...
describe('seed-lesson-7-4', () => {
  describe('Lesson 7-4: Graphing Rational Functions', () => {
    const lesson7_4Seed: SeedLesson = {
      unitNumber: 7,
    ...
    it('lesson has correct metadata', () => {
      expect(lesson7_4Seed.unitNumber).toBe(7);
      expect(lesson7_4Seed.title).toBe('Graphing Rational Functions');
      expect(lesson7_4Seed.slug).toBe('module-7-lesson-4');
      expect(lesson7_4Seed.orderIndex).toBe(4);
    });

    it('has exactly 11 phases', () => {
      expect(lesson7_4Seed.phases).toHaveLength(11);
    });
    ...
    it('correct phase sequence: explore, vocab, learn, 3xworked_example, learn, 2xworked_example, discourse, reflection', () => {
      ...
      const actualSequence = lesson7_4Seed.phases.map((p) => p.phaseType);
      expect(actualSequence).toEqual(expectedSequence);
    });

    it('explore phase is phase 1', () => {
      const explorePhase = lesson7_4Seed.phases.find((p) => p.phaseType === 'explore');
      expect(explorePhase?.phaseNumber).toBe(1);
    });

    it('vocabulary phase is phase 2', () => {
      const vocabPhase = lesson7_4Seed.phases.find((p) => p.phaseType === 'vocabulary');
      expect(vocabPhase?.phaseNumber).toBe(2);
    });

    it('first learn phase is phase 3', () => {
      const learnPhases = lesson7_4Seed.phases.filter((p) => p.phaseType === 'learn');
    ...
    it('discourse phase is phase 10', () => {
      const discoursePhase = lesson7_4Seed.phases.find((p) => p.phaseType === 'discourse');
      expect(discoursePhase?.phaseNumber).toBe(10);
    });

    it('reflection phase is phase 11', () => {
      const reflectionPhase = lesson7_4Seed.phases.find((p) => p.phaseType === 'reflection');
      expect(reflectionPhase?.phaseNumber).toBe(11);
    });

    it('has exactly 5 worked_example phases', () => {
      const workedExamples = lesson7_4Seed.phases.filter((p) => p.phaseType === 'worked_example');
  ...
  describe('activity records for lesson 7-4', () => {
