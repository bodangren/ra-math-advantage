...
describe('seed-lesson-6-3', () => {
  describe('Lesson 6-3: Common Logarithms', () => {
    const lesson6_3Seed: SeedLesson = {
      unitNumber: 6,
    ...
    it('lesson has correct metadata', () => {
      expect(lesson6_3Seed.unitNumber).toBe(6);
      expect(lesson6_3Seed.title).toBe('Common Logarithms');
      expect(lesson6_3Seed.slug).toBe('module-6-lesson-3');
      expect(lesson6_3Seed.orderIndex).toBe(3);
    });

    it('has exactly 12 phases', () => {
      expect(lesson6_3Seed.phases).toHaveLength(12);
    });
    ...
    it('correct phase sequence: explore, vocab, learn, 4xworked_example, learn, 2xworked_example, discourse, reflection', () => {
      ...
      const actualSequence = lesson6_3Seed.phases.map((p) => p.phaseType);
      expect(actualSequence).toEqual(expectedSequence);
    });

    it('explore phase is phase 1', () => {
      const explorePhase = lesson6_3Seed.phases.find((p) => p.phaseType === 'explore');
      expect(explorePhase?.phaseNumber).toBe(1);
    });

    it('vocabulary phase is phase 2', () => {
      const vocabPhase = lesson6_3Seed.phases.find((p) => p.phaseType === 'vocabulary');
      expect(vocabPhase?.phaseNumber).toBe(2);
    });

    it('first learn phase is phase 3', () => {
      const learnPhases = lesson6_3Seed.phases.filter((p) => p.phaseType === 'learn');
    ...
    it('discourse phase is phase 11', () => {
      const discoursePhase = lesson6_3Seed.phases.find((p) => p.phaseType === 'discourse');
      expect(discoursePhase?.phaseNumber).toBe(11);
    });

    it('reflection phase is phase 12', () => {
      const reflectionPhase = lesson6_3Seed.phases.find((p) => p.phaseType === 'reflection');
      expect(reflectionPhase?.phaseNumber).toBe(12);
    });

    it('has exactly 6 worked_example phases', () => {
      const workedExamples = lesson6_3Seed.phases.filter((p) => p.phaseType === 'worked_example');
  ...
  describe('activity records for lesson 6-3', () => {
