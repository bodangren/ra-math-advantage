...
describe('seed-lesson-3-3', () => {
  describe('Lesson 3-3: Proving Polynomial Identities', () => {
    const lesson3_3Seed: SeedLesson = {
      unitNumber: 3,
    ...
    it('lesson has correct metadata', () => {
      expect(lesson3_3Seed.unitNumber).toBe(3);
      expect(lesson3_3Seed.title).toBe('Proving Polynomial Identities');
      expect(lesson3_3Seed.slug).toBe('module-3-lesson-3');
      expect(lesson3_3Seed.orderIndex).toBe(3);
    });

    it('has exactly 7 phases', () => {
      expect(lesson3_3Seed.phases).toHaveLength(7);
    });
    ...
    it('correct phase sequence: explore, vocab, learn, 2×worked_example, discourse, reflection', () => {
      ...
      const actualSequence = lesson3_3Seed.phases.map((p) => p.phaseType);
      expect(actualSequence).toEqual(expectedSequence);
    });

    it('explore phase is phase 1', () => {
      const explorePhase = lesson3_3Seed.phases.find((p) => p.phaseType === 'explore');
      expect(explorePhase?.phaseNumber).toBe(1);
    });

    it('vocabulary phase is phase 2', () => {
      const vocabPhase = lesson3_3Seed.phases.find((p) => p.phaseType === 'vocabulary');
      expect(vocabPhase?.phaseNumber).toBe(2);
    });

    it('learn phase is phase 3', () => {
      const learnPhase = lesson3_3Seed.phases.find((p) => p.phaseType === 'learn');
      expect(learnPhase?.phaseNumber).toBe(3);
    });

    it('discourse phase is phase 6', () => {
      const discoursePhase = lesson3_3Seed.phases.find((p) => p.phaseType === 'discourse');
      expect(discoursePhase?.phaseNumber).toBe(6);
    });

    it('reflection phase is phase 7', () => {
      const reflectionPhase = lesson3_3Seed.phases.find((p) => p.phaseType === 'reflection');
      expect(reflectionPhase?.phaseNumber).toBe(7);
    });

    it('has exactly 2 worked_example phases', () => {
      const workedExamples = lesson3_3Seed.phases.filter((p) => p.phaseType === 'worked_example');
  ...
  describe('activity records for lesson 3-3', () => {
