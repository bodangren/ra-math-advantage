...
describe('seed-lesson-4-3', () => {
  describe('Lesson 4-3: nth Roots and Rational Exponents', () => {
    const lesson4_3Seed: SeedLesson = {
      unitNumber: 4,
    ...
    it('lesson has correct metadata', () => {
      expect(lesson4_3Seed.unitNumber).toBe(4);
      expect(lesson4_3Seed.title).toBe('nth Roots and Rational Exponents');
      expect(lesson4_3Seed.slug).toBe('module-4-lesson-3');
      expect(lesson4_3Seed.orderIndex).toBe(3);
    });

    it('has exactly 12 phases', () => {
      expect(lesson4_3Seed.phases).toHaveLength(12);
    });
    ...
    it('correct phase sequence: explore, vocab, learn, 2×worked_example, learn, 4×worked_example, discourse, reflection', () => {
      ...
      const actualSequence = lesson4_3Seed.phases.map((p) => p.phaseType);
      expect(actualSequence).toEqual(expectedSequence);
    });

    it('explore phase is phase 1', () => {
      const explorePhase = lesson4_3Seed.phases.find((p) => p.phaseType === 'explore');
      expect(explorePhase?.phaseNumber).toBe(1);
    });

    it('vocabulary phase is phase 2', () => {
      const vocabPhase = lesson4_3Seed.phases.find((p) => p.phaseType === 'vocabulary');
      expect(vocabPhase?.phaseNumber).toBe(2);
    });

    it('first learn phase is phase 3', () => {
      const learnPhases = lesson4_3Seed.phases.filter((p) => p.phaseType === 'learn');
    ...
    it('discourse phase is phase 11', () => {
      const discoursePhase = lesson4_3Seed.phases.find((p) => p.phaseType === 'discourse');
      expect(discoursePhase?.phaseNumber).toBe(11);
    });

    it('reflection phase is phase 12', () => {
      const reflectionPhase = lesson4_3Seed.phases.find((p) => p.phaseType === 'reflection');
      expect(reflectionPhase?.phaseNumber).toBe(12);
    });

    it('has exactly 6 worked_example phases', () => {
      const workedExamples = lesson4_3Seed.phases.filter((p) => p.phaseType === 'worked_example');
  ...
  describe('activity records for lesson 4-3', () => {
