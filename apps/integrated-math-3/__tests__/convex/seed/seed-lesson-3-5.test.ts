...
describe('seed-lesson-3-5', () => {
  describe('Lesson 3-5: Roots and Zeros', () => {
    const lesson3_5Seed: SeedLesson = {
      unitNumber: 3,
    ...
    it('lesson has correct metadata', () => {
      expect(lesson3_5Seed.unitNumber).toBe(3);
      expect(lesson3_5Seed.title).toBe('Roots and Zeros');
      expect(lesson3_5Seed.slug).toBe('module-3-lesson-5');
      expect(lesson3_5Seed.orderIndex).toBe(5);
    });

    it('has exactly 11 phases', () => {
      expect(lesson3_5Seed.phases).toHaveLength(11);
    });

    it('correct phase sequence: explore, vocab, learn, 2×worked_example, learn, 3×worked_example, discourse, reflection', () => {
      ...
      const actualSequence = lesson3_5Seed.phases.map((p) => p.phaseType);
      expect(actualSequence).toEqual(expectedSequence);
    });

    it('explore phase is phase 1', () => {
      const explorePhase = lesson3_5Seed.phases.find((p) => p.phaseType === 'explore');
      expect(explorePhase?.phaseNumber).toBe(1);
    });

    it('vocabulary phase is phase 2', () => {
      const vocabPhase = lesson3_5Seed.phases.find((p) => p.phaseType === 'vocabulary');
      expect(vocabPhase?.phaseNumber).toBe(2);
    });

    it('first learn phase is phase 3', () => {
      const learnPhases = lesson3_5Seed.phases.filter((p) => p.phaseType === 'learn');
    ...
    it('has exactly 5 worked_example phases', () => {
      const workedExamples = lesson3_5Seed.phases.filter((p) => p.phaseType === 'worked_example');
    ...
    it('discourse phase is phase 10', () => {
      const discoursePhase = lesson3_5Seed.phases.find((p) => p.phaseType === 'discourse');
      expect(discoursePhase?.phaseNumber).toBe(10);
    });

    it('reflection phase is phase 11', () => {
      const reflectionPhase = lesson3_5Seed.phases.find((p) => p.phaseType === 'reflection');
      expect(reflectionPhase?.phaseNumber).toBe(11);
    });
  });

  describe('activity records for lesson 3-5', () => {
