...
describe('seed-lesson-8-3', () => {
  describe('Lesson 8-3: Analyzing Population Data', () => {
    const lesson8_3Seed: SeedLesson = {
      unitNumber: 8,
    ...
    it('lesson has correct metadata', () => {
      expect(lesson8_3Seed.unitNumber).toBe(8);
      expect(lesson8_3Seed.title).toBe('Analyzing Population Data');
      expect(lesson8_3Seed.slug).toBe('module-8-lesson-3');
      expect(lesson8_3Seed.orderIndex).toBe(3);
    });

    it('has exactly 8 phases', () => {
      expect(lesson8_3Seed.phases).toHaveLength(8);
    });

    it('correct phase sequence: explore, vocab, learn, 3xworked_example, discourse, reflection', () => {
      ...
      const actualSequence = lesson8_3Seed.phases.map((p) => p.phaseType);
      expect(actualSequence).toEqual(expectedSequence);
    });

    it('explore phase is phase 1', () => {
      const explorePhase = lesson8_3Seed.phases.find((p) => p.phaseType === 'explore');
      expect(explorePhase?.phaseNumber).toBe(1);
    });

    it('vocabulary phase is phase 2', () => {
      const vocabPhase = lesson8_3Seed.phases.find((p) => p.phaseType === 'vocabulary');
      expect(vocabPhase?.phaseNumber).toBe(2);
    });

    it('learn phase is phase 3', () => {
      const learnPhase = lesson8_3Seed.phases.find((p) => p.phaseType === 'learn');
      expect(learnPhase?.phaseNumber).toBe(3);
    });

    it('discourse phase is phase 7', () => {
      const discoursePhase = lesson8_3Seed.phases.find((p) => p.phaseType === 'discourse');
      expect(discoursePhase?.phaseNumber).toBe(7);
    });

    it('reflection phase is phase 8', () => {
      const reflectionPhase = lesson8_3Seed.phases.find((p) => p.phaseType === 'reflection');
      expect(reflectionPhase?.phaseNumber).toBe(8);
    });

    it('has exactly 3 worked_example phases', () => {
      const workedExamples = lesson8_3Seed.phases.filter((p) => p.phaseType === 'worked_example');
  ...
  describe('activity records for lesson 8-3', () => {
