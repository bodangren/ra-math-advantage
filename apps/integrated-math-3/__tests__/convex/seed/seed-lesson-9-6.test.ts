...
describe('seed-lesson-9-6', () => {
  describe('Lesson 9-6: Translations of Trigonometric Graphs', () => {
    const lesson9_6Seed: SeedLesson = {
      unitNumber: 9,
    ...
    it('lesson has correct metadata', () => {
      expect(lesson9_6Seed.unitNumber).toBe(9);
      expect(lesson9_6Seed.title).toBe('Translations of Trigonometric Graphs');
      expect(lesson9_6Seed.slug).toBe('module-9-lesson-6');
      expect(lesson9_6Seed.orderIndex).toBe(6);
    });

    it('has exactly 11 phases', () => {
      expect(lesson9_6Seed.phases).toHaveLength(11);
    });
    ...
    it('correct phase sequence: explore, vocab, learn, 2xworked_example, learn, 3xworked_example, discourse, reflection', () => {
      ...
      const actualSequence = lesson9_6Seed.phases.map((p) => p.phaseType);
      expect(actualSequence).toEqual(expectedSequence);
    });

    it('explore phase is phase 1', () => {
      const explorePhase = lesson9_6Seed.phases.find((p) => p.phaseType === 'explore');
      expect(explorePhase?.phaseNumber).toBe(1);
    });

    it('vocabulary phase is phase 2', () => {
      const vocabPhase = lesson9_6Seed.phases.find((p) => p.phaseType === 'vocabulary');
      expect(vocabPhase?.phaseNumber).toBe(2);
    });

    it('first learn phase is phase 3', () => {
      const learnPhases = lesson9_6Seed.phases.filter((p) => p.phaseType === 'learn');
    ...
    it('discourse phase is phase 10', () => {
      const discoursePhase = lesson9_6Seed.phases.find((p) => p.phaseType === 'discourse');
      expect(discoursePhase?.phaseNumber).toBe(10);
    });

    it('reflection phase is phase 11', () => {
      const reflectionPhase = lesson9_6Seed.phases.find((p) => p.phaseType === 'reflection');
      expect(reflectionPhase?.phaseNumber).toBe(11);
    });

    it('has exactly 5 worked_example phases', () => {
      const workedExamples = lesson9_6Seed.phases.filter((p) => p.phaseType === 'worked_example');
  ...
  describe('activity records for lesson 9-6', () => {
