...
function requireReactServerDomWebpackClient_edge_production() {
  ...
  var ReactDOMSharedInternals = ReactDOM2.__DOM_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE, REACT_ELEMENT_TYPE = Symbol.for("react.transitional.element"), REACT_LAZY_TYPE = Symbol.for("react.lazy"), MAYBE_ITERATOR_SYMBOL = Symbol.iterator;
  function getIteratorFn(maybeIterable) {
    if (null === maybeIterable || "object" !== typeof maybeIterable) return null;
    maybeIterable = MAYBE_ITERATOR_SYMBOL && maybeIterable[MAYBE_ITERATOR_SYMBOL] || maybeIterable["@@iterator"];
    return "function" === typeof maybeIterable ? maybeIterable : null;
...
function useSearchParams() {
  ...
  }, getSearchParamsSnapshot, () => _getServerContext()?.searchParams ?? new URLSearchParams());
}
...
async function handleSsr(rscStream, navContext, fontData) {
  ...
  try {
    ...
    const htmlStream = await server_edgeExports.renderToReadableStream(root, {
      bootstrapScriptContent,
      onError(error) {
    ...
    const { renderToStaticMarkup } = await Promise.resolve().then(() => server_edge$1);
    const { createElement, Fragment } = await Promise.resolve().then(() => React$1);
    let insertedHTML = "";
