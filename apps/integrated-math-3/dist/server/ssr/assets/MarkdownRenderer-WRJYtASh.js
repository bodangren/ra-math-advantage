import { j as jsxRuntimeExports } from "./index-2LvcjxbV.js";
import { i, c as cn } from "./page-zHZ-yfGO.js";
import { h as getDefaultExportFromCjs } from "../index.js";
...
function MathBlock({ math, className = "" }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `math-block my-4 flex justify-center ${className}`, children: /* @__PURE__ */ jsxRuntimeExports.jsx(i, { block: true, math }) });
}
function MathInline({ math, className = "" }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `math-inline ${className}`, children: /* @__PURE__ */ jsxRuntimeExports.jsx(i, { math }) });
}
function parseMathSegments(content2) {
  const segments = [];
  const blockMathRe = /\$\$([\s\S]*?)\$\$|\\\[([\s\S]*?)\\\]/g;
  let lastIndex = 0;
  let match;
  while ((match = blockMathRe.exec(content2)) !== null) {
    if (match.index > lastIndex) {
      const textBefore = content2.slice(lastIndex, match.index);
  ...
  if (lastIndex < content2.length) {
    segments.push(...splitInlineMath(content2.slice(lastIndex)));
  }
  return segments;
}
function splitInlineMath(text2) {
  const segments = [];
  const inlineMathRe = /(?<!\$)\$(?!\$)([\s\S]*?)(?<!\$)\$(?!\$)/g;
  let lastIndex = 0;
  let match;
  while ((match = inlineMathRe.exec(text2)) !== null) {
    if (match.index > lastIndex) {
      segments.push({ type: "markdown", content: text2.slice(lastIndex, match.index) });
    }
    segments.push({ type: "inline-math", content: match[1].trim() });
    lastIndex = match.index + match[0].length;
  }
  if (lastIndex < text2.length) {
    segments.push({ type: "markdown", content: text2.slice(lastIndex) });
  }
  return segments;
}
const PROSE_CLASSES = [
...
const mdComponents = {
  ...
  table: ({ children }) => /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "overflow-x-auto my-5 rounded-lg border border-border", children: /* @__PURE__ */ jsxRuntimeExports.jsx("table", { className: "m-0 w-full text-sm", children }) }),
  thead: ({ children }) => /* @__PURE__ */ jsxRuntimeExports.jsx("thead", { className: "bg-primary/10 text-primary", children }),
  th: ({ children }) => /* @__PURE__ */ jsxRuntimeExports.jsx("th", { className: "px-4 py-2.5 text-left font-semibold text-xs uppercase tracking-wider border-b border-border", children }),
  td: ({ children }) => /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-4 py-2.5 border-b border-border/50", children }),
  tr: ({ children }) => /* @__PURE__ */ jsxRuntimeExports.jsx("tr", { className: "even:bg-muted/30 hover:bg-muted/50 transition-colors", children })
};
function MarkdownRenderer({ content: content2, className }) {
  const segments = parseMathSegments(content2);
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: cn(PROSE_CLASSES, className), children: segments.map((segment, index2) => {
    if (segment.type === "block-math") {
      return /* @__PURE__ */ jsxRuntimeExports.jsx(MathBlock, { math: segment.content }, index2);
    }
    if (segment.type === "inline-math") {
      return /* @__PURE__ */ jsxRuntimeExports.jsx(MathInline, { math: segment.content }, index2);
    }
...
function LessonMarkdownRenderer({ content: content2, className }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: cn("textbook-content", className), children: /* @__PURE__ */ jsxRuntimeExports.jsx(MarkdownRenderer, { content: content2 }) });
}
export {
