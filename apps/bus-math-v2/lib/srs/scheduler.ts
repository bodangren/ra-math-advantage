...
export function legacyToSrsCardState(card: {
  _id?: string;
  studentId: string;
  problemFamilyId: string;
  card: Record<string, unknown>;
  ...
  const stateMap: Record<number, SrsCardState['state']> = {
    0: 'new',
    1: 'learning',
    2: 'review',
    3: 'relearning',
  ...
  return {
    cardId: card._id ?? `legacy_${Date.now()}`,
    studentId: card.studentId,
    objectiveId: card.problemFamilyId,
    problemFamilyId: card.problemFamilyId,
    stability: (card.card.stability as number) ?? 0,
...
export function srsCardStateToLegacy(card: SrsCardState): {
  card: {
    due: string;
    stability: number;
    difficulty: number;
  ...
  return {
    ...
    due: new Date(card.dueDate).getTime(),
    lastReview: card.lastReview ? new Date(card.lastReview).getTime() : 0,
