...
describe('StudentError', () => {
  ...
  it('renders error fallback with reset button', () => {
    const reset = vi.fn();
    const error = new Error('Student route failed');

    render(<StudentError error={error} reset={reset} />);
  ...
  it('logs error to console', () => {
    ...
    render(<StudentError error={error} reset={vi.fn()} />);

    expect(consoleErrorSpy).toHaveBeenCalledWith('Student route error:', error);
  });
});
