...
describe('Phase 4: Units 5-6 Curriculum Audit', () => {
  const manifest = buildPublishedCurriculumManifest();
  const unit5Lessons = manifest.lessons.filter(l => l.unitNumber === 5);
  const unit6Lessons = manifest.lessons.filter(l => l.unitNumber === 6);

  it('Unit 5 has 11 lessons', () => {
    expect(unit5Lessons.length).toBe(11);
  });

  it('Unit 6 has 11 lessons', () => {
    expect(unit6Lessons.length).toBe(11);
  });

  it('Unit 5 lesson titles contain expected content', () => {
    const expectedContent = [
      ['Launch', 'Asset', 'Lifecycle'],
      ...
      ['Project', 'Sprint', 'Scope', 'Asset'],
      ['Project', 'Sprint', 'Lifecycle', 'Workbook'],
      ['Project', 'Sprint', 'Present', 'Recommendation'],
      ['Summative', 'Asset', 'Accounting', 'Mastery'],
    ];

    unit5Lessons.forEach((lesson, index) => {
  ...
  it('Unit 6 lesson titles contain expected content', () => {
    const expectedContent = [
      ...
      ['Project', 'Sprint', 'Scope', 'Cost'],
      ['Project', 'Sprint', 'Cost', 'Workbook'],
      ['Project', 'Sprint', 'Present', 'Margin'],
      ['Summative', 'Costing', 'Mastery'],
    ];

    unit6Lessons.forEach((lesson, index) => {
      const keywords = expectedContent[index];
      keywords.forEach(keyword => {
        expect(lesson.title.toLowerCase(), `Lesson ${index + 1}: missing keyword "${keyword}"`).toContain(keyword.toLowerCase());
      });
  ...
  it('Unit 5 lessons have valid phase structure', () => {
  ...
  it('Unit 6 lessons have valid phase structure', () => {
  ...
  it('Unit 5 lessons have no placeholder or TODO text in sections', () => {
    ...
    const allSections = unit5Lessons.flatMap(lesson =>
  ...
  it('Unit 6 lessons have no placeholder or TODO text in sections', () => {
    const placeholderPatterns = [
      /TODO/i,
      /FIXME/i,
      /XXX/i,
    ...
    const allSections = unit6Lessons.flatMap(lesson =>
      lesson.phases.flatMap(phase => phase.sections)
    );
  ...
  it('Unit 5 activities are registered and have valid schemas', () => {
    unit5Lessons.forEach(lesson => {
  ...
  it('Unit 6 activities are registered and have valid schemas', () => {
    unit6Lessons.forEach(lesson => {
      lesson.activities.forEach(activity => {
        expect(
          activityRegistry[activity.componentKey as keyof typeof activityRegistry],
  ...
  it('Unit 5 and 6 lessons have proper metadata', () => {
    [...unit5Lessons, ...unit6Lessons].forEach(lesson => {
