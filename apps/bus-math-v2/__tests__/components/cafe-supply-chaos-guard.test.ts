...
const componentPath = path.resolve(
  process.cwd(),
  'components/activities/simulations/CafeSupplyChaos.tsx',
);

describe('CafeSupplyChaos double-submit guard', () => {
  ...
  it('checks submittedRef.current before proceeding in handleNextDay', () => {
    expect(source).toMatch(/handleNextDay[\s\S]{0,200}if\s*\(\s*submittedRef\.current\s*\)\s*return/);
  });
