...
const componentPath = path.resolve(
  process.cwd(),
  'components/activities/simulations/ScenarioSwitchShowTell.tsx',
);

describe('ScenarioSwitchShowTell double-submit guard', () => {
  ...
  it('checks submittedRef.current before proceeding in handleComplete', () => {
    expect(source).toMatch(/handleComplete[\s\S]{0,300}if\s*\(\s*submittedRef\.current\s*\)\s*return/);
  });
