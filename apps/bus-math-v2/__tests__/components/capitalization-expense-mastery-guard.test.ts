...
const componentPath = path.resolve(
  process.cwd(),
  'components/activities/exercises/CapitalizationExpenseMastery.tsx',
);

describe('CapitalizationExpenseMastery double-submit guard', () => {
  ...
  it('resets submittedRef.current in handleNewScenario', () => {
    expect(source).toMatch(
      /handleNewScenario[\s\S]*?submittedRef\.current\s*=\s*false/,
    );
  });
});
