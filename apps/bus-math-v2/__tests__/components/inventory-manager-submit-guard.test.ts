...
const componentPath = path.resolve(
  process.cwd(),
  'components/activities/simulations/InventoryManager.tsx',
);

describe('InventoryManager double-submit guard', () => {
  ...
  it('checks submittedRef.current before proceeding in handleSubmit', () => {
    expect(source).toMatch(/handleSubmit[\s\S]{0,300}if\s*\(\s*submittedRef\.current\s*\)\s*return/);
  });
  ...
  it('resets submittedRef.current in resetGame', () => {
    expect(source).toMatch(
      /resetGame[\s\S]*?submittedRef\.current\s*=\s*false/,
    );
  });
});
